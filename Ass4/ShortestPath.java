...
public class ShortestPath{
	...
	*/
	static int ShortestPath(int[][] G){
		...
		int totalWeight = 0;
		/* ... Your code here ... */
		Comparator<item> comparator = new ECP();
		PriorityQueue<item> Q = new PriorityQueue<item>(numVerts,comparator);
		double[] D = new double[numVerts];
		Q.add(new item(0,D[0]));
		for(int i = 1;i < numVerts;i++){
			D[i] = Double.POSITIVE_INFINITY;
			Q.add(new item(i,D[i]));
		}
		...
		while(Q.size() > 0){
			...
			for(int i = 0; i < numVerts;i++){
				if(G[u.e][i] > 0){
					if(D[u.e] + G[u.e][i] < D[i]){
						Q.remove(new item(i,D[i]));
						D[i] = D[u.e] + G[u.e][i];
						Q.add(new item(i,D[i]));
					}
				}
...
class ECP implements Comparator<item>{
...
class item{
	int e;	//vertex i in graph
	double path;
	item(int e, double path){
		this.e = e;
		this.path = path;
	}
	item(){}
