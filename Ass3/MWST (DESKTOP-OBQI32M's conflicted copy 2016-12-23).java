...
public class MWST{
	...
	*/
	static int MWST(int[][] G){
		...
		//this is the "tree" it will just hold which vertices are actually in the MWST since we only need to calculate the weight and dont really care about the tree itself
		//it has size numVerts - 1 because that is a property of trees
		//might need to make this an array of elements
		int[] T = new int[numVerts-1];
		int size = 0;
		WeightedQuickUnion UF = new WeightedQuickUnion(numVerts);
		int totalWeight = 0;
...
class Element{
	...
	int b;
	int weight;
	Element(int a, int b, int weight){
		this.a = a;
		this.b = b;
		this.weight = weight;
	}
...
class Heap{ 
	...
	//takes an element e and gives back the weight of its edge
